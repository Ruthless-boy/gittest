 ...
 *      *
 *      * @param s  代表socket
 *      * @param response  代表传入的response对象
 *      * @throws IOException
 *
 * private static void handle200(Socket s, Response response) throws IOException {
 *         //先拿到响应的格式，现在写死为text/html
 *         String contentType = response.getContentType();
 *         //拿到响应头
 *         String headText = Constant.response_head_202;
 *         //headText中有个{}，用于格式化中替代文本，此时headText就是响应行
 *         headText = StrUtil.format(headText, contentType);
 *         //响应头从string转化为byte数组
 *         byte[] head = headText.getBytes();
 *         //getBody方法获取response的body属性
 ...
 *
 *
 *     prepareSession(Request request, Response response)方法：
 *     通过 cookie拿到 jsessionid, 然后通过 SessionManager 创建 session, 并且设置在 requeset 上
 */
...
public class HttpProcessor {
    public void execute(Socket s, Request request, Response response){
        try {
            ...
            //设置session，每次请求都会设置session，如果request的请求头中有了session对应的cookie，
            // 那就从SessionManager存储的sessionMap中获取，如果没有则新建一个session给request。
            prepareSession(request, response);

            Context context = request.getContext();

            //uri是资源名称，比如index.html，获取此资源对应的servlet，这行代码其实在这里没用
            String servletClassName = context.getServletClassName(uri);
            //根据uri访问资源的不同，workingServlet也不同。
    ...
    public void prepareSession(Request request, Response response) {
        //获取到的可能为null，说明是第一次访问
        String jsessionid = request.getJSessionIdFromCookie();
        //如果为null仍然传入，此方法会新建一个session，并返回
